  ...
  useEffect(() =>{
    ...
    if(dataToShow.length){
      ...
      tempData.map((searchData) => {
        ...
        const isEqualRecvDate = formattedRecvDate.some((element) => recvDate.includes(element));
        const isEqualEndDate = formattedEndDate.some((element) => endDate.includes(element));
        ...
        if(isEqualRecvDate && isEqualEndDate && 
          (searchAccomName.toLowerCase() === searchData.accomsName.slice(0,searchAccomName.length).toLowerCase() || searchAccomName === '')){
      ...
      <div className="search-bar">
        <div className="search-bar__input-wrapper">
          <div className="search-bar__input-wrapper__item">
            ...
            <input placeholder="Nhập tên chỗ nghỉ, địa điểm" value={searchAccomName}
              onChange={(e) => {
        ...
        <Button className="cyan" preIcon={() => <GrPowerReset size={20} stroke="#fff" fill="#fff" />}>
          Đặt lại
        </Button>
      </div>
